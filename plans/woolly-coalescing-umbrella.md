# Fix: Style Remix Generator Agent - Missing Browser Tools

## Problem

The agent was created but doesn't have access to Chrome browser automation tools. The `tools` field only specifies:
```
tools: Read, Write, Bash
```

But it needs the MCP Chrome tools to interact with Gemini.

## Fix

Update line 33 of `/Users/raghavbajoria/.claude/agents/design/style-remix-generator.md`:
...
# Original Plan: Style Remix Generator Agent

## Overview

Create a new agent called **"style-remix-generator"** that chains three existing skills to enable style-transfer image generation:

1. **Gemini Image Prompter** - Extract JSON prompt from user's source image
2. **Style Prompt Creator** - Apply extracted style to user's new subject
3. **Gemini Image Generator** - Generate the final image

## Agent Details

- **Name**: `style-remix-generator`
- **Location**: `/Users/raghavbajoria/.claude/agents/design/style-remix-generator.md`
- **Category**: design (fits with visual/creative agents)

## Workflow

```
┌─────────────────────────────────────────────────────────────────────────┐
│  1. START: Ask user for image file path                                 │
├─────────────────────────────────────────────────────────────────────────┤
│  2. EXTRACT: Upload image to Gemini → Get JSON prompt                   │
│     (Gemini Image Prompter logic)                                       │
├─────────────────────────────────────────────────────────────────────────┤
│  3. STYLE: Show extracted style → Ask user questions about new subject  │
│     (Style Prompt Creator logic)                                        │
├─────────────────────────────────────────────────────────────────────────┤
│  4. GENERATE: Take JSON prompt → Generate in Gemini with Nano Banana Pro│
│     (Gemini Image Generator logic)                                      │
├─────────────────────────────────────────────────────────────────────────┤
│  5. DONE: Image generated, no further action                            │
└─────────────────────────────────────────────────────────────────────────┘
```
...
| File | Purpose |
|------|---------|
| `/Users/raghavbajoria/.claude/agents/design/style-remix-generator.md` | Main agent definition |
...
1. Invoke by asking to "remix an image style" or "create in this style"
2. Provide a file path when prompted
3. Verify the agent:
   - Successfully validates image file exists
   - Uploads to Gemini and extracts JSON prompt
   - Shows extracted style and asks questions
   - Generates combined JSON + text prompt
   - Successfully submits to Gemini for generation
