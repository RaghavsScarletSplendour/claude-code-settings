...
**Resolve the target URL.** If no URL/domain was given and you're running inside a website's repo, auto-detect the production URL before asking cold — check `next.config.*`, `vercel.json`, the `<loc>` entries in `sitemap.xml`, `package.json` `homepage`, or the deploy config — then confirm the detected domain with the user. Don't hand-dig the repo for the domain every single run.
...
**Always fetch fresh.** Do not reuse a crawl from a previous run or a cached dump in `/tmp` — a re-run must re-fetch live, or the findings silently reflect a stale snapshot (a fixed issue still shows as broken, or vice-versa). If you deliberately reuse a fetch within a single run, stamp it with the fetch time and warn before scoring if it's more than ~1 hour old.
...
**Count rendered content only.** When tallying H1s, images, word count, etc., **exclude HTML comments (`<!-- ... -->`) and `display:none` / `hidden` nodes** — commented-out or hidden legacy markup inflates these counts and produces wrong findings (e.g. "5 H1s" when the page renders one). If the rendered count differs materially from the raw-source count, report both and **score on the rendered count.**
...
Save the report files to the **current working directory** unless the user names a different folder. Pick the paths once and reuse them everywhere below, with the domain hyphenated and an ISO date — e.g. `./seo-audit-example-com-2025-03-13.docx` and `.../seo-audit-example-com-2025-03-13.pdf`. Always pass absolute paths to tools so nothing depends on where a sub-step runs. (This skill runs on the user's own machine — there is no cloud sandbox `/sessions/.../mnt/outputs/` directory, so never write there.)

### Setup

**Do not run `npm install` as a separate step.** Check whether `docx` is already available first, and only install if missing. Do this in a single combined bash command so it counts as one tool call. Install locally into the working directory (not globally) so it works without elevated permissions:

```bash
node -e "require('docx')" 2>/dev/null || npm install docx
```

If the local `require` can't find it after a local install, set `NODE_PATH` to the working dir's `node_modules` when you run the generator script, or `cd` into the directory that has `node_modules` first.
...
Write to the output path you chose in **Output location** above (absolute path preferred). Use a filename like `seo-audit-example-com-2025-03-13.docx` (domain with hyphens, ISO date).

### Validate

Confirm the file was written and is a valid Office Open XML package (a `.docx` is a ZIP — this checks it parses):

```bash
node -e "const fs=require('fs');const f='./seo-audit-[domain]-[date].docx';const s=fs.statSync(f);console.log('OK',f,s.size,'bytes');" && unzip -l "./seo-audit-[domain]-[date].docx" >/dev/null && echo "valid docx"
```

If this fails, inspect the error, fix the JS, and regenerate.

### Convert to PDF

The `.docx` is the primary deliverable; the PDF is a convenience. Try LibreOffice first (direct `docx`→`pdf`), then fall back to **headless Chrome** (which a stock Mac has, LibreOffice usually doesn't), and only skip as a last resort.

Because Chrome can't read `.docx`, when LibreOffice is absent you must **also emit an HTML version of the report** — mirror the same sections + scores as the `.docx` — to `./seo-audit-[domain]-[date].html`, then print that to PDF:

```bash
DOC="./seo-audit-[domain]-[date].docx"; PDF="./seo-audit-[domain]-[date].pdf"; HTML="./seo-audit-[domain]-[date].html"
if command -v soffice >/dev/null 2>&1; then
  soffice --headless --convert-to pdf "$DOC" --outdir "."
elif command -v libreoffice >/dev/null 2>&1; then
  libreoffice --headless --convert-to pdf "$DOC" --outdir "."
else
  # No LibreOffice → print the HTML report (emit it first) via headless Chrome.
  CHROME="/Applications/Google Chrome.app/Contents/MacOS/Google Chrome"
  [ -x "$CHROME" ] || CHROME="$(command -v google-chrome || command -v chromium || echo '')"
  if [ -n "$CHROME" ] && [ -f "$HTML" ]; then
    "$CHROME" --headless=new --disable-gpu --no-pdf-header-footer \
      --print-to-pdf="$PDF" "file://$(pwd)/${HTML#./}"
  else
    echo "No LibreOffice and no Chrome/HTML — shipping .docx only. (brew install --cask libreoffice for direct PDF.)"
  fi
fi
```

So the PDF works on a stock Mac without LibreOffice. Deliver whatever succeeded — the `.docx` always, the `.pdf` (and `.html`) when available; never treat a missing PDF as a hard failure.

### Deliver to the user

Give the user the real file path(s) on their machine. (`present_files` and `computer://` links are cloud-sandbox features and don't exist here.) Show absolute paths so they're easy to find:

```
Your audit report is ready:
- Word doc: /absolute/path/to/seo-audit-[domain]-[date].docx
- PDF:      /absolute/path/to/seo-audit-[domain]-[date].pdf   (only if LibreOffice produced it)
```
