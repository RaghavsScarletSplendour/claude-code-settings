---
name: search-first
description: 'Use this skill when a user describes coding their own version of a problem that mature open-source libraries, frameworks, or platform primitives already handle well. Trigger phrases: "going to implement," "about to write," "rolling my own," "plan is to," "starting to code," "by hand," or naming a mechanism they intend to build (Levenshtein/fuzzy search, token-bucket rate limiting, HMAC/signature verification, CSV/encoding/date parser, DB-table-plus-cron job queue, retries, caching, auth, validation). Trigger even under deadline pressure ("ship next week"), and when a stated plan or new feature quietly assumes hand-built plumbing. Also trigger when debugging flaky tests or integration failures (payments, webhooks, third-party APIs) where a maintained tool or known fix likely exists. The point: find the proven solution before custom code is written. Skip conceptual "how does X work" questions, wiring up an already-chosen library, and ordinary UI/CRUD against existing endpoints.'
metadata:
  origin: ECC
---
